...
class CellposeModel(UnetModel):
    ...
    def __init__(self, gpu=False, pretrained_model=False, 
        ...
        if model_type is not None or (pretrained_model and not os.path.exists(pretrained_model[0])):
            ...
            pretrained_model = [model_path(pretrained_model_string, j, self.torch) for j in model_range] #选择预训练模型
        ...
        if self.pretrained_model:
            self.net.load_model(self.pretrained_model[0], device=self.device) # 加载预训练模型
            # print('self.pretrained_model[0]', self.pretrained_model[0])

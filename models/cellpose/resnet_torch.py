...
class upsample(nn.Module):
    ...
    def get_features(self, style, xd, mkldnn=False):
        xu = []
        x = self.up[-1](xd[-1], xd[-1], style, mkldnn=mkldnn)
        xu.append(x)
        for n in range(len(self.up)-2,-1,-1):
...
class CPnet(nn.Module):
    ...
    def get_features(self, data):
        ...
        if self.mkldnn:
            style = self.make_style(xd[-1].to_dense()) 
        else:
            style = self.make_style(xd[-1])
        if not self.style_on:
            style = style * 0

        xu = self.upsample.get_features(style, xd, self.mkldnn)
        out = self.output(xu[-1]) # 输出层之后的，即cellpose模型输出
